...
def create_grayjay_structure(output_dir):
    os.makedirs(os.path.join(output_dir, "stores"), exist_ok=True)
    
    # exportInfo
    with open(os.path.join(output_dir, "exportInfo"), "w", encoding="utf-8") as f:
        json.dump({"version": "1"}, f, separators=(',', ':'))
    ...
    with open(os.path.join(output_dir, "plugin_settings"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(output_dir, "plugins"), "w", encoding="utf-8") as f:
        json.dump(plugins, f, separators=(',', ':'))

def convert_freetube_to_grayjay(db_path, out_dir):
    playlists = []
    with open(db_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            name = playlist.get("playlistName", "Unknown")
            for video in playlist.get("videos", []):
                url = f"https://www.youtube.com/watch?v={video.get('videoId')}"
                playlists.append(f"{name}:::{generate_uuid()}\n{url}")

    stores_path = os.path.join(out_dir, "stores", "Playlists")
    with open(stores_path, "w", encoding="utf-8") as f:
        f.write(json.dumps(playlists, ensure_ascii=False))

def pack_zip(out_dir, zip_name="grayjay-export.zip"):
...
def main():
    db_path = "freetube-playlists.db"
    output_dir = "grayjay-export"

    if not os.path.exists(db_path):
        print("Error: freetube-playlists.db not found.")
        return

    create_grayjay_structure(output_dir)
    convert_freetube_to_grayjay(db_path, output_dir)
    pack_zip(output_dir, "grayjay-export.zip")
